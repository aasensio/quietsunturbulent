   ...
   real(kind=8) :: dlogP2dmu1, dlogP2dmu2, dlogP2dsigma1, dlogP2dsigma2, dlogP2dp, dlogP2dB
	...
	integer :: i, j, loop, ierr
		...
		do i = 1, 7
			...
			loop = loop + 1
		enddo
		xB2(1) = sigmoid(xB2(1), priorLower(7), priorUpper(7))   ! mu1_B
		xB2(2) = sigmoid(xB2(2), priorLower(8), priorUpper(8))   ! sigma1_B
		xB2(3) = sigmoid(xB2(3), priorLower(9), priorUpper(9))   ! mu_V
		xB2(4) = sigmoid(xB2(4), priorLower(10), priorUpper(10))   ! sigma_V
		xB2(5) = sigmoid(xB2(5), priorLower(11), priorUpper(11))   ! mu2_B
		xB2(6) = sigmoid(xB2(6), priorLower(12), priorUpper(12))   ! sigma2_B
		xB2(7) = sigmoid(xB2(7), 0.d0, 1.d0)                       ! p
...
!---------------------
! Log-normal mixture prior for B
!---------------------
		do i = 1, lineList%nActiveLines
			logP1 = log(xB2(7)) - log(xB2(2)) - log(B2(i)) - (log(B2(i)) - xB2(1))**2 / (2.d0*xB2(2)**2)
			logP2 = log(1.d0-xB2(7)) - log(XB2(6)) - log(B2(i)) - (log(B2(i)) - xB2(5))**2 / (2.d0*xB2(6)**2)
						
			logP = logP + logSum(logP1, logP2)
									
			dlogPdlogP1 = 1.d0 / (1.d0 + exp(-logP1+logP2))
			dlogPdlogP2 = 1.d0 / (1.d0 + exp(logP1-logP2))
			
			dlogP1dmu1 = (log(B2(i)) - xB2(1) ) / xB2(2)**2
			dlogP1dmu2 = 0.d0
			dlogP1dsigma1 = -1.d0 / xB2(2) + (log(B2(i)) - xB2(1) )**2 / xB2(2)**3
			dlogP1dsigma2 = 0.d0
			dlogP1dp = 1.d0 / xB2(7)
			dlogP1dB = -1.d0 / B2(i) - (log(B2(i)) - xB2(1) ) / (B2(i)*xB2(2)**2)
			
			dlogP2dmu1 = 0.d0
			dlogP2dmu2 = (log(B2(i)) - xB2(5) ) / xB2(6)**2
			dlogP2dsigma1 = 0.d0
			dlogP2dsigma2 = -1.d0 / xB2(2) + (log(B2(i)) - xB2(5) )**2 / xB2(6)**3
			dlogP2dp = -1.d0 / (1.d0-xB2(7))
			dlogP2dB = -1.d0 / B2(i) - (log(B2(i)) - xB2(5) ) / (B2(i)*xB2(6)**2)
			
								
! dMix/dB_i
			logPGradient(3*lineList%nActiveLines+i) = logPGradient(3*lineList%nActiveLines+i) + dlogPdlogP1 * dlogP1dB + dlogPdlogP2 * dlogP2dB
! dMix/dmu1
			logPGradient(6*lineList%nActiveLines+1) = logPGradient(6*lineList%nActiveLines+1) + dlogPdlogP1 * dlogP1dmu1 + dlogPdlogP2 * dlogP2dmu1
! dMix/dsigma1
			logPGradient(6*lineList%nActiveLines+2) = logPGradient(6*lineList%nActiveLines+2) + dlogPdlogP1 * dlogP1dsigma1 + dlogPdlogP2 * dlogP2dsigma1			
! dMix/dmu2
			logPGradient(6*lineList%nActiveLines+5) = logPGradient(6*lineList%nActiveLines+5) + dlogPdlogP1 * dlogP1dmu2 + dlogPdlogP2 * dlogP2dmu2
! dMix/dsigma2
			logPGradient(6*lineList%nActiveLines+6) = logPGradient(6*lineList%nActiveLines+6) + dlogPdlogP1 * dlogP1dsigma2 + dlogPdlogP2 * dlogP2dsigma2
! dMix/dp
			logPGradient(6*lineList%nActiveLines+7) = logPGradient(6*lineList%nActiveLines+7) + dlogPdlogP1 * dlogP1dp + dlogPdlogP2 * dlogP2dp
...
! Jeffreys prior for sigma
  		logP = logP - 3.d0*log(xB2(6))

! dP/dsigma
  		logPGradient(6*lineList%nActiveLines+6) = logPGradient(6*lineList%nActiveLines+6) - 3.d0 / xB2(6)
...
! Use chain rule to take into account the sigmoid transformation
		...
		enddo
		logPGradient(6*lineList%nActiveLines+7) = logPGradient(6*lineList%nActiveLines+7) * diffSigmoid(trial(6*lineList%nActiveLines+7), 0.d0, 1.d0)   ! p
...
!------------------------------------------------
   ...
   real(kind=8) :: v, xwrite(4)
	integer i
		...
		enddo
		do i = 1, 6
			x2(6*lineList%nActiveLines+i) = sigmoid(x(6*lineList%nActiveLines+i), priorLower(7+i-1), priorUpper(7+i-1))
		enddo		
		x2(6*lineList%nActiveLines+7) = sigmoid(x(6*lineList%nActiveLines+7), 0.d0, 1.d0)
