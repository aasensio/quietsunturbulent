...
! Read input data
!------------------------------------------------
	subroutine readInput(atmosphere, lineList, atlas)
...
! Set-up line information
 		call setupLines(atmosphere,lineList,atlas)
...
!------------------------------------------------
	subroutine readAtmosphere(atmosphere)
	type(atmosphereType) :: atmosphere
	integer :: i
...
! Get the order so that it is ordered
...
!------------------------------------------------
	...
	type(lineListType) :: lineList
		...
		do i = 1, lineList%nLines
			read(13,*) lineList%transition(i)%lambda0, lineList%transition(i)%lambdaNew, lineList%transition(i)%lambdaKurucz, lineList%transition(i)%gf, lineList%transition(i)%Elow, &
...
!------------------------------------------------
	...
	real(kind=8) :: a, b, x1, y1, x2, y2
		...
		do i = 1, lineList%nLines
			...
			if (lineList%transition(i)%lambda0 /= 0) then
...
! Get ranges of the line
				...
				write(*,FMT='(A,I3,A,I7,A,I7,A,F8.3,A,F8.3)') 'Line ', i, ' - Range : ', limits(1), ' -> ', limits(2), ' - lambda: ', atlas%lambda(limits(1)), ' -> ', atlas%lambda(limits(2))
