...
!-----------------------------------------------------------------
	subroutine calculate_abundance_Pg_from_T_Pe(height_in, temper_in, Pe_in, &
		PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out, P_total)
	integer :: n_grid
	real(kind=8) :: height_in(:), temper_in(:), Pe_in(:)
	real(kind=8) :: initial_values(22)
	real(kind=8), dimension(size(height_in)) :: PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out, P_total
...
! 		deallocate(which_included)
	...
	end subroutine calculate_abundance_Pg_from_T_Pe
...
!  PH2plus_out : partial pressure of H2+ molecules (dyn/cm^2)
!-----------------------------------------------------------------
	subroutine calculate_abundance_Pg_from_T_Pe_single(height_in, temper_in, Pe_in, &
		PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out, P_total)
	integer :: n_grid
	real(kind=8) :: height_in, temper_in, Pe_in
	real(kind=8) :: initial_values(22)
	real(kind=8) :: PH_out, PHminus_out, PHplus_out, PH2_out, PH2plus_out, P_total
	character(len=40) :: abundance_file
	real(kind=8) :: mole(273), height, minim_ioniz
	integer :: i, ind, l, loop	, minim, loop_mol, nmolec
...
! Now extract also the partial pressure from H, H-, H+, H2 and H2+
		PH_out = x_sol(1)			
		PHminus_out = x_sol(1) * P_elec / equilibrium_atomic(2,1)
		PHplus_out = x_sol(1) / P_elec * equilibrium_atomic(1,1)
		if (temper < 1.d5) then
			PH2_out = x_sol(1)**2 / equilibrium(1)
			PH2plus_out = x_sol(1)**2 / P_elec * (equilibrium_atomic(1,1) / equilibrium(2))
		else
			PH2_out = 0.d0
			PH2plus_out = 0.d0
		endif
...
! 		deallocate(which_included)
	...
	end subroutine calculate_abundance_Pg_from_T_Pe_single
